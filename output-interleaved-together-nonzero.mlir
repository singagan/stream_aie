builtin.module {
  aie.device(npu1_1col) {
    ...
    aiex.runtime_sequence(%0 : memref<64x64xi16>, %1 : memref<64x64xi16>, %2 : memref<64x64xi16>) {
      ...
      aiex.npu.dma_memcpy_nd(%0[0, 0, 0, 0][1, 2, 32, 32][0, 1, 64, 1]) {id = 0 : i64, issue_token = true, metadata = @of_00_I} : memref<64x64xi16>
      ...
      aiex.npu.dma_wait {symbol = @of_00_I}
      aiex.npu.dma_memcpy_nd(%0[0, 0, 0, 32][1, 2, 32, 32][0, 1, 64, 1]) {id = 0 : i64, issue_token = true, metadata = @of_00_I} : memref<64x64xi16>
      ...
      // first input tile to 02 and 03:
      aiex.npu.dma_wait {symbol = @of_00_I}
      aiex.npu.dma_memcpy_nd(%0[0, 0, 0, 2048][1, 2, 32, 32][0, 1, 64, 1]) {id = 0 : i64, issue_token = true, metadata = @of_00_I} : memref<64x64xi16>

      // second input tile to 02 and 03:
      aiex.npu.dma_wait {symbol = @of_00_I}
      aiex.npu.dma_memcpy_nd(%0[0, 0, 0, 2080][1, 2, 32, 32][0, 1, 64, 1]) {id = 0 : i64, issue_token = true, metadata = @of_00_I} : memref<64x64xi16>
